...
class App:
    ...
    MITM_CMD = ["python3", "attacks/servermitm.py"]
    ...
    def create_progressbar(self, parent_frame, tank_name):
        ...
        progressbar = ttk.Progressbar(dynamic_pregressbar, 
                                    ...
                                    length=150,
                                    mode='determinate')
        status_label = ttk.Label(dynamic_pregressbar, text="Stato: In attesa")
        ...
        while True:
            try:
                ...
                for pvar, statlab in zip(progress_var_list, status_label_list):
                    pvar.set(0)
                    statlab.config(text="Stato: In attesa")
                client1.close()
                client2.close()
                client3.close()
                obf_client.close()
    ...
    def start_mitm(self):
        ...
        if attack_key == "mitm":
            ...
            self.addresses, self.ports, mitm_ip, mitm_port = self.parse_config_file("config.ini")
    ...
    def stop_attack(self):
        ...
        ips = self.parse_config_file_for_iptables("config.ini", plc_names[0], plc_names[1])
        result2=subprocess.run(['iptables', '-t', 'nat', '-D', 'PREROUTING', '-i', 'eth0', '-s', ips[1], '-d', ips[0], '-j', 'DNAT', '--to-destination', ips[2]])
        self.btn_stop["state"] = "disabled"
