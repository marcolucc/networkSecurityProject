...
struct stationInfo
{
	...
	int port; //aggiunta per porta dinamica 
};
		...
		while (getline(cfgfile, line))
		{
			...
			if (line_str[0] != '#' && strlen(line_str) > 1)
			{
				...
				else if (!strncmp(line_str, "station", 7))
				{
					...
					else if (!strncmp(functionType, "port", 4))
					{
						char temp_buffer[100];
						getData(line_str, temp_buffer, '"', '"');
						stations_info[stationNumber].port = atoi(temp_buffer);
...
void *exchangeDataWithPLC(void *args)
{
	...
	int socket_fd, port = stations_info[stationNumber].port;
	// inserimento porta dinamico
	//int socket_fd, port = porta[stationNumber];
	struct sockaddr_in server_addr;
	struct hostent *server;
	int data_len;
	socklen_t cli_len;
	...
	while (1)
	{
		...
		//printf("send %d, socket %d, port: %d \n ", data_len, socket_fd, stations_info[stationNumber].port);
		...
		else
		{
			...
			while (data_len == 0)
			{
				...
				data_len = recvfrom(socket_fd, localBuffer, sizeof(*localBuffer), 0, (struct sockaddr *)&server_addr, &cli_len);
			        printf("recv %d, socket %d port: %d\n ", data_len, socket_fd, stations_info[stationNumber].port);
				counter++;
				if (data_len == -1)
				{
					perror("Error receiving data: " );
					//printf("\n ip: %s port: %d  \n" ,  stations_info[stationNumber].ip , stations_info[stationNumber].port);
			...
			if (data_len < 0)
			{
				printf("Error receiving data on socket %d %s:%d\n", socket_fd, stations_info[stationNumber].ip, port);
			}
			else
			{
				/*
...
void connectToPLCStations()
{
	for (int i = 0; i < num_stations; i++)
	{
		...
		cout << endl << stations_info[i].port << endl ;
...
int main()
{
	...
	{
		pthread_mutex_lock(&bufferLock);
		printf("Station 1\nLevel: %d \t\tPump: %d\t\tValve: %d\n", stations_data[0].analogIn[0], stations_data[0].digitalOut[0], stations_data[0].digitalOut[1]);
		printf("Station 2\nLevel: %d\n", stations_data[1].analogIn[0]);
		printf("Station 3\nLevel: %d \t\tRequest: %d\n", stations_data[2].analogIn[0], stations_data[2].digitalOut[0]);
		pthread_mutex_unlock(&bufferLock);
		sleep_ms(3000);
	}
}
